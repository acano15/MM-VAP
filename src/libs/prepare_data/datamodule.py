...
class VapDataModule(pl.LightningDataModule):
    def __init__(
        ...
        if self.num_workers > 0:
            self._kwargs["prefetch_factor"] = int(self.batch_size / 2)

        self.load_data()

    def load_data(self):
        if self.train_path is None:
            assert self.train_path is None, "Train path is NONE."
            assert exists(self.train_path), f"No TRAIN file found: {self.train_path}"
        if self.val_path is None:
            assert self.val_path is None, "Validation path is NONE."
            assert exists(self.val_path), f"No VAL file found: {self.val_path}"

        if self.test_path is None:
            assert self.test_path is None, "Test path is NONE."
    ...
    def train_dataloader(self):
        if self._train_loader is None:
            self._train_loader = DataLoader(self.train_dset, **self._kwargs)
        return self._train_loader

    def val_dataloader(self):
        if self._val_loader is None:
            self._val_loader = DataLoader(self.val_dset, **self._kwargs)
        return self._val_loader

    def test_dataloader(self):
        if self._test_loader is None:
            self._test_loader = DataLoader(self.test_dset, **self._kwargs)
