...
from src.libs.utils import vad_list_to_onehot, vad_list_to_onehot_windowed
...
class VapDataset(Dataset):
    def __init__(
        ...
        if shuffle:
            grouped = list(self.df.groupby('id'))
            ...
            self.df = pd.concat(shuffled_groups, ignore_index=True)
        ...
        if self.use_cache:
            ...
            if not self.cache_csv.exists():
                ...
                try:
                    ...
                    keys_iterator = TqdmIterator(keys, self.lock, desc=f'Creating cache - {self.split} set')
    ...
    def preload(self, cache_lookup = None, audio_dict = None, image_dict = None):
        ...
        if audio_dict == None or len(audio_dict) == 0:
        ...
        if image_dict == None or len(image_dict) == 0:
    ...
    def __getitem__(self, idx: int) -> Dict[str, Union[Tensor, str, int, float, None]]:        
        ...
        if self.use_cache:
            ...
            if self.multimodal:
                ...
                #    d["vad_list"], duration=duration + self.horizon, frame_hz=self.frame_hz
                #)
                vad = vad_list_to_onehot_windowed(
                    d["vad_list"],
                    duration=duration + self.horizon,
                    start_time=d["start"],
                    end_time=d["end"],
                ...
                face1 = face1.contiguous()
                body1 = body1.contiguous()
                face_im1 = face_im_1.contiguous()
                gaze2 = gaze2.contiguous()
                head2 = head2.contiguous()
                face2 = face2.contiguous()
                body2 = body2.contiguous()
                face_im2 = face_im_2.contiguous()
        ...
        with self.lock:
            ...
            for i, candidate in enumerate(self.pool_availability):
                if candidate:
                    ...
                    break
            pbar = tqdm(total=len(self.df_dict[key]), position=tqdm_position+1, desc='{} - {}'.format(tqdm_position, key), leave=True)
...
class TqdmIterator:
    ...
    def __init__(self, src, lock, position = 0, desc = ''):
        ...
        with self.lock:
            self.pbar = tqdm(total=self.num, position = position, desc = desc, leave=True)
