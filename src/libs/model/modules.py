...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, dropout: float, bias: bool = False):
        ...
        if dim % num_heads != 0:
            candidates = [h for h in range(1, dim + 1) if dim % h == 0]
            if not candidates:
                raise ValueError(f"No valid num_heads found for dim={dim}")
            num_heads = min(candidates, key=lambda h: abs(h - num_heads))
            ...
            if self.mask is None or self.mask.shape[-1] < T:
                ...
                if self.context_limit > 0:
                    for j in range(mask.shape[2]):
                        del_mask_start = 0
                        del_mask_end = int(max(0, j - self.context_limit + 1))
