...
from scripts.face_extractor import CFaceExtractor
...
# DEFINITIONS
NOXI_SESSIONS = ["Paris", "Nottingham", "Augsburg", "Arab", "Esp", "Ita", "Indi"]
NOXIJ_SESSIONS = ["Jap", "Chin"]
...
def filter_by_keyword(src, keyword):
    ...
    for row in tqdm(src, desc=f'Filtering by {keyword}'):
        if keyword.lower() == "noxi":
            if any(k in row[0] for k in NOXI_SESSIONS):
                tgt.append(row)
        elif keyword.lower() == "noxij":
            if any(k in row[0] for k in NOXIJ_SESSIONS):
                tgt.append(row)
        elif keyword.lower() == "all":
            tgt.append(row)
        elif keyword in row[0]:
            tgt.append(row)
    return tgt


def _match_session_keyword(path: Path, keyword: str) -> bool:
    parent_str = str(path.parent).lower()
    keyword_l = keyword.lower()

    if keyword_l == "noxi":
        return any(k.lower() in parent_str for k in NOXI_SESSIONS)

    if keyword_l == "noxij":
        return any(k.lower() in parent_str for k in NOXIJ_SESSIONS)

    if keyword_l == "all":
        return True

    return keyword_l in parent_str
...
def main(cfg_dict: DictConfig):
    ...
    data_list_keyword = filter_by_keyword(data_list[1:], keyword)
    ...
    files_to_extract = [x for x in session_folder.glob("**/*.mp4")
                        if x.is_file() and not x.parent.name[0].isdigit() and
                        _match_session_keyword(x, keyword)]
