...
def load_vad_csv(src_path, delimiter = ','):
    ...
    for row in src_data:
        if row[-1] == '[speech]':
           if 'left' in str(src_path.absolute()):
...
def main(cfg_dict: DictConfig):
    ...
    tgt_dir = Path(cfg_dict.dataset.extracted_path)
    ...
    subjects_folder = (src_dir / "subjects").resolve()
    files = [x for x in subjects_folder.glob('**/*') if x.is_file() and not x.parent.name[
        0].isdigit()]
    ...
    logger.info(f'Found {len(files)} files in {subjects_folder}')
    ...
    for file in tqdm(files, desc='Collecting data src'):
        ...
        conversation_id = separated[-2]
        subject_id = separated[-3]
        file_id = conversation_id + '_' + subject_id

        # input()
        if not (file_id in data_dict.keys()):

            data_dict[file_id] = {}
            data_dict[file_id]['subject'] = subject_id
            data_dict[file_id]['conversation'] = conversation_id
            data_dict[file_id]['dataset'] = 'mmhri'

        if file_name.endswith(".wav"):
            if 'left' in file_name:
                tgt_path = tgt_dir / '{}-{}'.format(file_id, file_name)
                logger.dev(f'Processing left audio file: {tgt_path}')
            ...
            if 'right' in file_name:
                tgt_path = tgt_dir / '{}-{}'.format(file_id, file_name)
                logger.dev(f'Processing right audio file: {tgt_path}')
            ...
            tmp_file_name = tmp_file_name.replace('left', 'mix')
            tmp_file_name = tmp_file_name.replace('right', 'mix')
            ...
            if (('audio_path1' in data_dict[file_id].keys()) and
                ...
                if not tgt_path.exists():
                    wav1 = AudioSegment.from_wav(data_dict[file_id]['audio_path1']).set_channels(1)
                    wav2 = AudioSegment.from_wav(data_dict[file_id]['audio_path2']).set_channels(1)
            ...
            if file_name.startswith("non_varbal_"):
                if 'left' in file_name:
                    ...
                    logger.dev(f'Processing left non-verbal face file: {tgt_path}')
                    ...
                    logger.dev(f'Processing left non-verbal gaze file: {tgt_path}')
                    ...
                    logger.dev(f'Processing left non-verbal head file: {tgt_path}')
                    ...
                    logger.dev(f'Processing left non-verbal body file: {tgt_path}')
                    data_dict[file_id]['body_path1'] = str(tgt_path.absolute())
                    if not tgt_path.exists():
                        write_csv(tgt_path, body_data)
                else:
                    tgt_path = tgt_dir / '{}-face_{}'.format(file_id, file_name.split('_')[-1])
                    logger.dev(f'Processing right non-verbal face file: {tgt_path}')
                    data_dict[file_id]['face_path2'] = str(tgt_path.absolute())
                    if not tgt_path.exists():
                        write_csv(tgt_path, face_data)

                    tgt_path = tgt_dir / '{}-gaze_{}'.format(file_id, file_name.split('_')[-1])
                    logger.dev(f'Processing right non-verbal gaze file: {tgt_path}')
                    data_dict[file_id]['gaze_path2'] = str(tgt_path.absolute())
                    if not tgt_path.exists():
                        write_csv(tgt_path, gaze_data)

                    tgt_path = tgt_dir / '{}-head_{}'.format(file_id, file_name.split('_')[-1])
                    logger.dev(f'Processing right non-verbal head file: {tgt_path}')
                    data_dict[file_id]['head_path2'] = str(tgt_path.absolute())
                    if not tgt_path.exists():
                        write_csv(tgt_path, head_data)

                    tgt_path = tgt_dir / '{}-body_{}'.format(file_id, file_name.split('_')[-1])
                    logger.dev(f'Processing right non-verbal body file: {tgt_path}')
                    data_dict[file_id]['body_path2'] = str(tgt_path.absolute())
                    if not tgt_path.exists():
                        write_csv(tgt_path, body_data)
        elif "vad" in file_name and file_name.endswith(".txt"):
            ...
            try:
                ...
                data_list.append(
                    [file_id,  # 0
                     ...
                     body_path2,
                     tmp_vad_list,  # 12
                     tmp_data_dict['subject'],
                     tmp_data_dict['conversation'],
    ...
    # 2. Extract faces from data
    files_to_extract = [x for x in subjects_folder.glob('**/*.mp4') if x.is_file() and
                        not x.parent.name[0].isdigit()]
    ...
    file_ids = list({row[0] for row in data_list[1:]})
    ...
    tgt_train_path = tgt_dir / (base_name + f'train.csv')
    ...
    tgt_valid_path = tgt_dir / (base_name + f'valid.csv')
    ...
    tgt_test_path = tgt_dir / (base_name + f'test.csv')
    ...
    logger.info(
        'train_size: {:} ({:.2f})'.format(
            len(train_data) - 1, (len(train_data) - 1) / (len(data_list) - 1)))
    logger.info(
        'valid_size: {:} ({:.2f})'.format(
            len(valid_data) - 1, (len(valid_data) - 1) / (len(data_list) - 1)))
    logger.info(
        'test_size: {:} ({:.2f})'.format(
            len(test_data) - 1, (len(test_data) - 1) / (len(data_list) - 1)))
    ...
    for src_path in [tgt_train_path, tgt_valid_path, tgt_test_path]:
        ...
        for row in tqdm(src_list, desc=f'Processing rows from file {src_path}'):
            add_row = True
            left_name = f"{row[0].replace('_subject_','-sub_')}_conv_{row[0].split('_')[1]}_left.npy"
            tmp_tgt_path = tgt_dir / left_name
            if os.path.exists(tmp_tgt_path):
                try:
                    if tmp_tgt_path not in cache_face_checked.keys():
                        logger.dev(f'Loading left face from {tmp_tgt_path}. Checking if it is valid')
            ...
            right_name = (f"{row[0].replace('_subject_','-sub_')}_conv_{row[0].split('_')[1]}_right.npy")
            tmp_tgt_path = tgt_dir / right_name
            if os.path.exists(tmp_tgt_path):
                try:
                    logger.dev(f'Loading right face from {tmp_tgt_path}. Checking if it is valid')
                    _ = np.load(tmp_tgt_path, allow_pickle=False, mmap_mode='r')
